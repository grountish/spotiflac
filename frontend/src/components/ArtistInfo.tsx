...
export function ArtistInfo({ artistInfo, albumList, trackList, searchQuery, sortBy, selectedTracks, downloadedTracks, failedTracks, skippedTracks, downloadingTrack, isDownloading, bulkDownloadType, downloadProgress, currentDownloadInfo, currentPage, itemsPerPage, downloadedLyrics, failedLyrics, skippedLyrics, downloadingLyricsTrack, checkingAvailabilityTrack, availabilityMap, downloadedCovers, failedCovers, skippedCovers, downloadingCoverTrack, isBulkDownloadingCovers, isBulkDownloadingLyrics, isMetadataLoading = false, sharedLocalPlayer, onSearchChange, onSortChange, onToggleTrack, onToggleSelectAll, onDownloadTrack, onDownloadSoulseekTrack, onDownloadLyrics, onDownloadCover, onCheckAvailability, onDownloadAllLyrics, onDownloadAllCovers, onDownloadAll, onDownloadSelected, onStopDownload, onOpenFolder, onAlbumClick, onArtistClick, onPageChange, onTrackClick, onBack, }: ArtistInfoProps) {
    const localCollection = useLocalCollection({
        collectionImageUrl: artistInfo.images,
        collectionKey: `artist:${artistInfo.name}`,
        collectionSubtitle: `${artistInfo.followers.toLocaleString()} followers`,
        collectionTitle: artistInfo.name,
      ...
      {activeTab === "tracks" && trackList.length > 0 && (<div className="space-y-4">
          <div className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              {hasLocalPlayback ? (<>
                  <Button onClick={() => void localCollection.playCollection()} size="sm" disabled={localCollection.isCheckingFiles || localCollection.playableTracks.length === 0}>
                    ...
                    {localCollection.isCurrentCollectionActive && localCollection.isPlaying ? "Pause All" : localCollection.allTracksDownloaded ? "Play All" : "Play Downloaded"}
                  </Button>
                  {remainingDownloadCount > 0 && (<Button onClick={onDownloadAll} size="sm" variant="secondary" disabled={isDownloading}>
                      {isDownloading && bulkDownloadType === "all" ? (<Spinner />) : (<Download className="h-4 w-4"/>)}
                      Download Remaining ({remainingDownloadCount.toLocaleString()})
                    </Button>)}
                  <Button onClick={() => void localCollection.deleteAll()} size="sm" variant="destructive" disabled={localCollection.isCheckingFiles || localCollection.playableTracks.length === 0}>
              ...
              {selectedTracks.length > 0 && !localCollection.allTracksDownloaded && (<Button onClick={onDownloadSelected} size="sm" variant="secondary" disabled={isDownloading}>
                  {isDownloading && bulkDownloadType === "selected" ? (<Spinner />) : (<Download className="h-4 w-4"/>)}
