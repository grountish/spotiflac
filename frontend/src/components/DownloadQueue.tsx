...
import { X, Download, CheckCircle2, XCircle, Clock, FileCheck, Trash2, HardDrive, Zap, Timer, FileDown, Play, Pause, SkipBack, SkipForward } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogHeader, DialogTitle, } from "@/components/ui/dialog";
import { Badge } from "@/components/ui/badge";
import { Progress } from "@/components/ui/progress";
import { useLocalAudioPlayer } from "@/hooks/useLocalAudioPlayer";
import { GetDownloadQueue, ClearCompletedDownloads, ClearAllDownloads, ExportFailedDownloads, ReadAudioFileAsDataURL } from "../../wailsjs/go/main/App";
...
export function DownloadQueue({ isOpen, onClose }: DownloadQueueProps) {
    ...
    const playableTracks = useMemo(() => queueInfo.queue
        .filter((item) => (item.status === "completed" || item.status === "skipped") && !!item.file_path)
        .map((item) => ({
        id: item.id,
    ...
    const currentTrackIndex = playableTracks.findIndex((track) => track.id === currentTrackId);
    ...
    <DialogContent className="max-w-[1200px] w-[95vw] max-h-[80vh] flex flex-col p-0 gap-0 [&>button]:hidden">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <div className="flex items-center gap-2">
                    {(item.status === "completed" || item.status === "skipped") && item.file_path && (<Button variant={currentTrackId === item.id && isPlaying ? "default" : "outline"} size="icon-sm" className="shrink-0" onClick={() => toggleTrack({
                    id: item.id,
                    title: item.track_name,
                    subtitle: [item.artist_name, item.album_name].filter(Boolean).join(" • "),
                    filePath: item.file_path,
                })} disabled={loadingTrackId === item.id}>
                        {loadingTrackId === item.id ? <Download className="h-3.5 w-3.5 animate-spin"/> : currentTrackId === item.id && isPlaying ? <Pause className="h-3.5 w-3.5"/> : <Play className="h-3.5 w-3.5"/>}
                      </Button>)}
      ...
      {playableTracks.length > 0 && (<div className="border-t bg-muted/20 px-6 py-4">
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <Button variant="outline" size="icon-sm" onClick={() => void playPrevious()} disabled={loadingTrackId !== null || currentTrackIndex <= 0}>
                <SkipBack className="h-3.5 w-3.5"/>
              </Button>
              <Button size="icon" onClick={() => void (currentTrack ? toggleTrack(currentTrack) : playableTracks[0] ? toggleTrack(playableTracks[0]) : Promise.resolve())} disabled={loadingTrackId !== null}>
                {loadingTrackId !== null ? <Download className="h-4 w-4 animate-spin"/> : currentTrack && isPlaying ? <Pause className="h-4 w-4"/> : <Play className="h-4 w-4"/>}
              </Button>
              <Button variant="outline" size="icon-sm" onClick={() => void playNext()} disabled={loadingTrackId !== null || currentTrackIndex < 0 || currentTrackIndex >= playableTracks.length - 1}>
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-4">
                <div className="min-w-0">
                  <p className="truncate text-sm font-medium">
                    {currentTrack ? currentTrack.title : "Ready to play downloaded tracks"}
                  </p>
                  <p className="truncate text-xs text-muted-foreground">
                    {currentTrack?.subtitle || `${playableTracks.length} downloaded track${playableTracks.length === 1 ? "" : "s"} available`}
                  </p>
                </div>
              ...
              <Progress className="mt-2 h-1.5" value={duration > 0 ? (currentTime / duration) * 100 : 0}/>
            </div>

            {currentTrack && (<Button variant="ghost" size="sm" className="text-xs" onClick={stopPlayback}>
                Stop
              </Button>)}
          </div>
        </div>)}
