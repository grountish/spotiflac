...
import { buildLocalCollectionPlaybackTargetFromFolder, buildLocalCollectionPlaybackTargetFromMetadata, resolveSpotifyUrlFromDownloadedFolder, type LocalCollectionPlaybackTarget } from "@/lib/local-playback";
...
function App() {
    ...
    const [historyPlayableTargets, setHistoryPlayableTargets] = useState<Record<string, LocalCollectionPlaybackTarget | null>>({});
    const [historyPlayableLookup, setHistoryPlayableLookup] = useState<Record<string, string>>({});
    const [historyPlayableCheckingIds, setHistoryPlayableCheckingIds] = useState<Set<string>>(new Set());
    const [isHistoryPlayableLookupReady, setIsHistoryPlayableLookupReady] = useState(false);
    ...
    const activeLocalCollectionRef = useRef<LocalCollectionPlaybackTarget | null>(null);
    useEffect(() => {
        activeLocalCollectionRef.current = activeLocalCollection;
    }, [activeLocalCollection]);
    const syncLocalCollection = useCallback((collection: LocalCollectionPlaybackTarget) => {
        if (activeLocalCollectionRef.current?.key !== collection.key) {
            return;
        }

        if (collection.tracks.length === 0) {
            localPlayer.setTrackList([]);
            setActiveLocalCollection(null);
            return;
        }

        setActiveLocalCollection(collection);
        localPlayer.setTrackList(collection.tracks);
    }, [localPlayer]);
    const activateLocalCollection = useCallback(async (collection: LocalCollectionPlaybackTarget, trackId?: string) => {
        if (collection.tracks.length === 0) {
            return;
        }

        const isSameCollection = activeLocalCollectionRef.current?.key === collection.key;
        if (!isSameCollection) {
            setActiveLocalCollection(collection);
            localPlayer.setTrackList(collection.tracks);
        }

        const fallbackTrackId = isSameCollection ? (trackId || localPlayer.currentTrackId || collection.tracks[0]?.id) : (trackId || collection.tracks[0]?.id);
        const targetTrack = collection.tracks.find((track) => track.id === fallbackTrackId) || collection.tracks[0];
        if (!targetTrack) {
            return;
        }

        await localPlayer.toggleTrack(targetTrack);
    }, [localPlayer]);
    const sharedLocalPlayer = useMemo(() => ({
        ...
        activateCollection: activateLocalCollection,
        syncCollection: syncLocalCollection,
        stopPlayback: localPlayer.stopPlayback,
    }), [activeLocalCollection?.key, activateLocalCollection, localPlayer.currentTrackId, localPlayer.isPlaying, localPlayer.loadingTrackId, localPlayer.stopPlayback, syncLocalCollection]);
    ...
    useEffect(() => {
        let cancelled = false;
        setIsHistoryPlayableLookupReady(false);

        const run = async () => {
            if (!fetchHistory.some((item) => item.type === "album" || item.type === "playlist")) {
                setHistoryPlayableLookup({});
                setIsHistoryPlayableLookupReady(true);
                return;
            }

            try {
                const cachedItems = await GetFetchHistory() as CachedFetchHistoryItem[];
                if (cancelled) {
                    return;
                }
                ...
                for (const item of sortedItems) {
                    ...
                    if ((normalizedType !== "album" && normalizedType !== "playlist") || !item.data) {
                        continue;
                    }

                    const historyType = normalizedType as HistoryItem["type"];
                    const key = getHistoryIdentityKey(historyType, item.url);
                    if (!nextLookup[key]) {
                        nextLookup[key] = item.data;
                    }
                }

                setHistoryPlayableLookup(nextLookup);
            }
            catch (error) {
                if (!cancelled) {
                    console.error("Failed to load fetch history cache:", error);
                }
            }
            finally {
                if (!cancelled) {
    ...
    useEffect(() => {
        setHistoryPlayableTargets({});
        setHistoryPlayableCheckingIds(new Set());
    }, [download.downloadedTracks.size, download.skippedTracks.size]);
    ...
    const resolveHistoryPlayableTarget = useCallback(async (item: HistoryItem) => {
        if (item.type !== "album" && item.type !== "playlist") {
            return null;
        }

        if (item.id in historyPlayableTargets) {
            return historyPlayableTargets[item.id];
        }

        if (historyPlayableCheckingIds.has(item.id)) {
            return null;
        }

        if (!isHistoryPlayableLookupReady) {
            return null;
        }

        const identityKey = getHistoryIdentityKey(item.type, item.url);
        const cachedData = historyPlayableLookup[identityKey];
        if (!cachedData) {
            setHistoryPlayableTargets((prev) => ({ ...prev, [item.id]: null }));
            return null;
        }

        setHistoryPlayableCheckingIds((prev) => {
            if (prev.has(item.id)) {
                return prev;
            }
        ...
        try {
            const parsed = JSON.parse(cachedData) as SpotifyMetadataResponse;
            const target = await buildLocalCollectionPlaybackTargetFromMetadata(parsed);
            setHistoryPlayableTargets((prev) => ({ ...prev, [item.id]: target }));
            return target;
        }
        catch (error) {
            console.error("Failed to resolve recent fetch playback target:", error);
            setHistoryPlayableTargets((prev) => ({ ...prev, [item.id]: null }));
            return null;
        }
        finally {
    ...
    }, [historyPlayableCheckingIds, historyPlayableLookup, historyPlayableTargets, isHistoryPlayableLookupReady]);
    const handleHistoryPlay = useCallback(async (item: HistoryItem) => {
        const target = await resolveHistoryPlayableTarget(item);
        if (!target || target.tracks.length === 0) {
            toast.info("No downloaded tracks found for this item yet");
            return;
        }

        await activateLocalCollection(target);
    }, [activateLocalCollection, resolveHistoryPlayableTarget]);
    const currentPlayableHistoryId = useMemo(
        () => Object.entries(historyPlayableTargets).find(([, target]) => target?.key === activeLocalCollection?.key)?.[0] ?? null,
        [activeLocalCollection?.key, historyPlayableTargets],
    );
    const playableHistoryIds = useMemo(
        () => new Set(Object.entries(historyPlayableTargets).filter(([, target]) => !!target && target.tracks.length > 0).map(([id]) => id)),
        [historyPlayableTargets],
    );
    ...
    const handleOpenDownloadedCollection = useCallback(async (folder: DownloadedFolderSummary) => {
        try {
            const url = await resolveSpotifyUrlFromDownloadedFolder(folder);
            if (!url) {
                toast.info(`Couldn't match ${folder.title} back to Spotify metadata`);
                return;
            }
            ...
            console.error("Failed to open downloaded collection:", error);
            toast.error(`Failed to open ${folder.title}`);
        }
    }, [metadata]);
                ...
                return (<>
                    ...
                    {!isSearchMode && !metadata.metadata && (
                        <DownloadedAlbumsCarousel
                            ...
                            onOpenCollection={(folder) => { void handleOpenDownloadedCollection(folder); }}
    ...
    return (<TooltipProvider>
        <div className="min-h-screen bg-background flex flex-col">
            <TitleBar />
            <Sidebar currentPage={currentPage} onPageChange={handlePageChange} onNavigateHome={handleNavigateHome}/>


            <div className="flex-1 ml-14 mt-10 p-4 md:p-8">
                <div className={showGlobalPlayer ? "mx-auto grid w-full max-w-[1600px] items-start gap-6 lg:grid-cols-[minmax(0,1fr)_320px] xl:grid-cols-[minmax(0,1fr)_340px]" : "mx-auto w-full max-w-[1600px] space-y-6"}>
                    <div className="min-w-0 space-y-6">
                        {renderPage()}
                    </div>
                    {showGlobalPlayer && activeLocalCollection && (
